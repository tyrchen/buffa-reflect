...
pub struct Builder {
    ...
    generate_view_reflection: Option<bool>,
    view_reflection_include_file: Option<String>,
    view_reflection_root_path: Option<String>,
}
...
impl Builder {
    ...
    #[must_use]
    pub fn generate_view_reflection(mut self, enabled: bool) -> Self {
        self.generate_view_reflection = Some(enabled);
        self
    }
    ...
    #[must_use]
    pub fn view_reflection_include_file(mut self, name: impl Into<String>) -> Self {
        self.view_reflection_include_file = Some(name.into());
        self
    }
    ...
    #[must_use]
    pub fn view_reflection_root_path(mut self, path: impl Into<String>) -> Self {
        self.view_reflection_root_path = Some(path.into());
        self
    }
    ...
    pub fn compile(self) -> Result<(), Error> {
        ...
        let want_view_refl = self
            .generate_view_reflection
            .unwrap_or(self.file_descriptor_set_bytes_expr.is_some());
        if want_view_refl {
            emit_view_reflection_file(&self, &fds, &out_dir)?;
        }
...
/// Emit `OUT_DIR/<view_reflection_include_file>` with one
/// `impl ReflectMessageView<'a>` block per generated view type.
fn emit_view_reflection_file(
    builder: &Builder,
    fds: &FileDescriptorSet,
    out_dir: &Path,
) -> Result<(), Error> {
    let file_name = builder
        .view_reflection_include_file
        .as_deref()
        .unwrap_or("_reflect_views.rs");
    let view_root = builder
        .view_reflection_root_path
        .as_deref()
        .unwrap_or("crate::__buffa::view");
    let bytes_expr = builder
        .file_descriptor_set_bytes_expr
        .as_deref()
        .ok_or_else(|| {
            Error::BuffaBuild(
                "view-reflection emit requires file_descriptor_set_bytes(\"...\") binding".into(),
            )
        })?;

    let mut out = String::new();
    out.push_str(&format!(
        "// @generated by buffa-reflect-build {} for view reflection. DO NOT EDIT.\n\
         // View module convention pinned to buffa-codegen as of buffa 0.4.x.\n\
         // If buffa changes its __buffa::view module layout, regenerate.\n",
        env!("CARGO_PKG_VERSION"),
    ));

    for entry in collect_view_entries(fds) {
        let view_path = format!(
            "{view_root}::{}{}View<'__a>",
            entry.module_path, entry.struct_name
        );
        let fqn = entry.full_name;
        out.push_str(&format!(
            r#"
impl<'__a> ::buffa_reflect::ReflectMessageView<'__a> for {view_path} {{
    fn descriptor(&self) -> ::buffa_reflect::MessageDescriptor {{
        ...
        pool.get_message_by_name("{fqn}")
            .expect(concat!("descriptor for `", "{fqn}", "` not found"))
    }}
}}
"#,
        ));
...
/// Per-message info needed to emit a view-reflection impl.
struct ViewEntry {
    /// Fully-qualified protobuf name (e.g. `acme.api.v1.Library.Book`).
    full_name: String,
    /// Rust module-path relative to the view root (`library::` for a
    /// nested message, empty for top-level).
    module_path: String,
    /// Rust struct base name (the `View` suffix is appended in the impl).
    struct_name: String,
}

fn collect_view_entries(fds: &FileDescriptorSet) -> Vec<ViewEntry> {
    let mut out = Vec::new();
    for file in &fds.file {
        let pkg = file.package.as_deref().unwrap_or("");
        for msg in &file.message_type {
            collect_view_entries_inner(msg, pkg, "", &mut out);
        }
    }
    out
}

fn collect_view_entries_inner(
    msg: &DescriptorProto,
    proto_scope: &str,
    rust_scope: &str,
    ...
    let full_name = if proto_scope.is_empty() {
        name.to_string()
    } else {
        format!("{proto_scope}.{name}")
    };
    ...
    if !is_map_entry {
        out.push(ViewEntry {
            ...
            module_path: rust_scope.to_string(),
            struct_name: name.to_string(),
        });
    }
    if !msg.nested_type.is_empty() {
        let child_rust_scope = format!("{rust_scope}{}::", to_snake_case(name));
        for nested in &msg.nested_type {
            collect_view_entries_inner(nested, &full_name, &child_rust_scope, out);
        }
    }
}

/// Lifted verbatim from `buffa-codegen::oneof::to_snake_case` so the
/// generated view-module path mirrors what buffa-codegen emits. Kept
/// in sync manually; if buffa changes the convention, the
/// version-pinned header in the emitted file flags the mismatch.
